...
from t5qa import T5ForQuestionAnswering, T5Model
...
def main():
    ...
    if "long-t5" in model_args.model_name_or_path or "longt5" in model_args.model_name_or_path:
        ...
        #model = T5ForQuestionAnswering.from_pretrained(
        #    "bert-base-uncased",
        #    from_tf=bool(".ckpt" in model_args.model_name_or_path),
        #    config=bert_config,
        #    real_config=config,
        ...
        #    use_auth_token=True if model_args.use_auth_token else None,
        #)
        #model.encoder = T5Model.from_pretrained(model_args.model_name_or_path).encoder
    ...
    question_column_name = "question_hubert_code" if "question_hubert_code" in column_names else column_names[0]
    context_column_name = "context_hubert_code" if "context_hubert_code" in column_names else column_names[1]
    answer_start_column_name = "code_start" if "code_start" in column_names else column_names[2]
    answer_end_column_name = "code_end" if "code_end" in column_names else column_names[3]
    ...
    def prepare_train_features(examples):
        ...
        if "t5" in model_args.model_name_or_path or "led" in model_args.model_name_or_path:
            ...
            qlens = [len(q) for q,c in zip(examples[question_column_name], examples[context_column_name])]
            clens = [len(c) for q,c in zip(examples[question_column_name], examples[context_column_name])]
        #print([len(q)+ len(c) for q,c in zip(examples[question_column_name], examples[context_column_name])])
        #examples["q"] = [tokenizer.convert_ids_to_tokens(x) for x in examples[question_column_name]]
        #examples["c"] = [tokenizer.convert_ids_to_tokens(x) for x in examples[context_column_name]]
        #print(examples["q"][0])
        ...
        #dummy = tokenizer("a","i was not pleased at how this works ",truncation="only_second", max_length = 7,stride=1, return_overflowing_tokens=True)
        #print(dummy.input_ids)
        #s()
        #tokenized_examples = tokenizer(
        #    examples["q" if pad_on_right else "c"],
        #    examples["c" if pad_on_right else "q"],
            ...
            if "t5" in model_args.model_name_or_path or "led" in model_args.model_name_or_path:
                ...
                while True:
                    ...
                    if ans_start >= start and ans_end <= end:
                        ...
                        tokenized_examples["input_ids"].pop()
                        tokenized_examples["attention_mask"].pop()
        ...
        #tokenized_examples["input_ids"] = torch.LongTensor(tokenized_examples["input_ids"])
        #tokenized_examples["attention_mask"] = torch.LongTensor(tokenized_examples["attention_mask"])
        #print(tokenized_examples.keys())
        #for k in tokenized_examples:
        #    print(len(tokenized_examples[k]))
        #print(len(tokenized_examples["input_ids"]),tokenized_examples["input_ids"][0][300:350],tokenized_examples["input_ids"][1][300:350])
        ...
        #print(tokenized_examples["attention_mask"].shape)
        #print(tokenized_examples["end_positions"].shape)
        #print(len(tokenized_examples["start_positions"]))
        ...
        #assert tokenized_examples["input_ids"].shape[0] == len(tokenized_examples["start_positions"])
        #s()
        #print(tokenized_examples.keys())
        ...
        #print(list(tokenized_examples["offset_mapping"][0]))
        #print(list(tokenized_examples["offset_mapping"][1]))

...
import torchaudio
import librosa
df = pd.read_parquet('./data/dev-00000-of-00001.parquet', engine='pyarrow')
df = df.dropna(axis=0, how='any', subset=["content_full_audio_path","question_audio_path"])
df = df.reset_index(drop=True)
speech, sample_rate = torchaudio.load(os.path.join("NMSQA_audio/dev_audios","context-0_0.mp3"))
speech, sample_rate = torchaudio.load(os.path.join("NMSQA_audio/dev_audios","context-0_0_0.mp3"))
speech, sample_rate = torchaudio.load(os.path.join("NMSQA_audio/dev_audios","context-0_0_1.mp3"))

with open(os.path.join("NMSQA_audio/dev_audios","context-0_0_0.lab"),"r") as f:
  data = f.readlines()
...
for i in tqdm(range(len(df))):
  for col in ["content_full_audio_path","question_audio_path"]:
    ...
    if col == "content_full_audio_path" :
      ...
      files = sorted([f for f in asr if df[col][i][:-4]+"_" in f and f.endswith(".mp3")],key = lambda x : int(x.split("-")[-1].split(".")[0]))
      print(files)
      #s()
      time = []
      transcription = ""
      offset = 0
      for f in files: 
        speech, sample_rate = torchaudio.load(os.path.join("NMSQA_audio/dev_audios",f))
...
df.to_parquet(f"./asr_results/{model_name}/dev_phone.parquet")

...
from dataset import STDataset,STDatasetSmall, collate_batch
...
def main(arg=None):
    ...
    class FreezingCallback(TrainerCallback):
        ...
        def on_epoch_begin(self, args: TrainingArguments, state: TrainerState, control: TrainerControl, **kwargs):
            if state.epoch < self.freeze_epoch:
                ...
                for name, param in self.freeze_model.named_parameters():
                    if name in release:
                        param.requires_grad = param.requires_grad = self.default_param_fix[name]
                    else:
    ...
    def parse_args(args):
        ...
        parser.add_argument("--SpeechMixEEDT52", action='store_true')
        ...
        parser.add_argument("--text_lna", action='store_true')
        parser.add_argument("--nlp_model_decoder_only", action = 'store_true')
    ...
    elif input_args['SpeechMixEEDT52']:
        model_type = "SpeechMixEEDT52"
        model = speechmix.SpeechMixEEDT52(**input_args)
    ...
    if 'custom_set' in input_args:
        ...
        if input_args['cache'] and os.path.exists(cache_path_train) and os.path.exists(cache_path_valid):
            ...
            _from = input_args["field"].split(".")[1]
            _to = input_args["field"].split(".")[2]
            _to_filter = input_args.get('filter', False)
            train_ds = STDataset(splt=input_args["train_split"],tokenizer=input_args["nlp_model_config"], translate_from = _from, translate_to = _to,_filter = _to_filter)
            valid_ds = STDataset(splt=input_args["test_split"],tokenizer=input_args["nlp_model_config"], translate_from = _from, translate_to = _to,_filter = _to_filter, size = 10)
    ...
    out_path = f"{input_args['speech_model_config']}_{input_args['nlp_model_config']}_{model_type}_{input_args.get('notes', '')}"
     
    training_args = TrainingArguments( #/work/twskvfb446
        output_dir= out_path,
        ...
        lr_scheduler_type = input_args.get('lr_scheduler_type', "linear"),
        save_total_limit=input_args.get('save_total_limit', 2),
        dataloader_num_workers=input_args.get('worker', 10),
        report_to="none",#"wandb" if input_args.get('wandb', True) else "none",
    ...
    if input_args["text_lna"]: 
    #for p in model.encoder_model.parameters():
    #    p.requires_grad = False
    #for xcoder in [model.encoder_model.named_parameters]:
    ...
    #            else:
    #                param.requires_grad = False
        for xcoder in [model.decoder_model.named_parameters]:
            for name, param in xcoder():
                #print("text",name)
                if param.requires_grad:
                    if any([k in name for k in {"Attention","layer_norm"}]):
                        ...
                        param.requires_grad = False
    decoder_grad_params = sum(param.numel() for param in model.decoder_model.parameters() if param.requires_grad)
    print(decoder_grad_params)
    #s()
    unpretrained = list(model.enc_to_dec_proj.parameters()) + list(model.length_adapters.parameters())
    #pretrained = [param for name, param in model.encoder_model.named_parameters() if "attention" in name or "layer_norm" in name]
    #pretrained += [param for name, param in model.decoder_model.named_parameters() if "Attention" in name or "layer_norm" in name]
    pretrained = list(model.encoder_model.parameters()) + list(model.decoder_model.parameters())
    #print("optimize",sum(p.numel() for p in pretrained),"parameters")
    # optimizer_grouped_parameters = [
    #     {
    #         "params": pretrained,
    #         "lr": input_args.get('lr', 5e-4),
    #     },
    #     {
    #         "params": unpretrained,
    #         "lr": 1e-3,
    ...
    trainer.train()#(os.path.join(out_path,"checkpoint-54036")))

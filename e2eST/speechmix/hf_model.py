...
class HFSpeechMixEED(PreTrainedModel):
    ...
    def forward(
        ...
        if decoder_text_prompt is not None and decoder_text_prompt_ids is not None:
            assert("Can only take str or input_ids as input, but not both")            
        elif decoder_text_prompt is not None:
        ...
        elif decoder_text_prompt_ids is not None:
            text_prompt = self.nlp_emb(decoder_text_prompt_ids.to(self.device))
            inputs_embeds = torch.cat((text_prompt.expand(inputs_embeds.shape[0], -1, -1), inputs_embeds), 1)
        else:
            pass
        #print("before cal_loss")
        ...
        print(sum(param.numel() for param in self.encoder_model.parameters() if param.requires_grad)) 
...
class HFSpeechMixEED3(PreTrainedModel):
    main_input_name = "input_values"
    ...
    def __init__(
        ...
        config = SpeechMixConfig.from_configs(speech_model_config,
                                              nlp_model_config)
        super(HFSpeechMixEED3, self).__init__(config)
    ...
    def forward(
        ...
        if decoder_text_prompt_ids is None and labels is None:
            decoder_input_ids = handle_decoder_input_none(
                self.decoder_model.config,
                encoder_outputs.last_hidden_state.shape[0],
                device=self.device,
        ...
        print("after w2v2",inputs_embeds.shape)
        if self.weighted_sum:
            # weighted sum
            stacked_feature = torch.stack(encoder_outputs["hidden_states"],
                                          dim=0)
        ...
        if return_model_detail:
            return_dict["shape_before_enc_dec_projector"] = inputs_embeds.shape
        inputs_embeds = self.enc_to_dec_proj(inputs_embeds)
        print("after proj",inputs_embeds.shape)
        if return_model_detail:
            return_dict["shape_after_enc_dec_projector"] = inputs_embeds.shape
        if decoder_text_prompt_ids is not None:
            text_prompt = self.nlp_emb(decoder_text_prompt_ids.to(self.device))
                #self.tokenizer(decoder_text_prompt,
                #               return_tensors="pt")["input_ids"].to(
                #    self.device))
            print(inputs_embeds.shape, text_prompt.shape)
...
class HFSpeechMixEED2(PreTrainedModel):
    main_input_name = "speech_input"
    ...
    def forward(self, speech_input=None, input_text_prompt=None, text_prompt_ids=None, text_input_ids=None, decoder_input_ids=None,
                labels=None,
        ...
        return_dict = {}
        if encoder_outputs is None:
            encoder_outputs = self.encoder_model(speech_input, output_hidden_states=True)
        ...
        if decoder_input_ids is None and labels is None:
            ...
            decoder_input_ids = shift_tokens_right(labels, self.decoder_model.config.pad_token_id,
                                                   self.decoder_model.config.decoder_start_token_id)
        inputs_embeds = encoder_outputs.last_hidden_state.to(self.device)
        print(inputs_embeds.shape, self.weighted_sum)
        ...
        if return_model_detail:
            return_dict['shape_after_enc_dec_projector'] = inputs_embeds.shape
        if text_prompt_ids is not None and input_text_prompt is not None:
            assert("Cannot have both text input and id inputs" == False)
        elif input_text_prompt is not None:
            text_prompt = self.nlp_emb(
                self.tokenizer(input_text_prompt, return_tensors='pt')['input_ids'].to(self.device))
            inputs_embeds = torch.cat((text_prompt, inputs_embeds), 1)
        elif text_prompt_ids is not None:
            text_prompt = self.nlp_emb(
                text_prompt_ids.to(self.device))
            print("shapes",text_prompt.shape, inputs_embeds.shape)
            inputs_embeds = torch.cat((text_prompt, inputs_embeds), 1)
        print("out",inputs_embeds.shape)
